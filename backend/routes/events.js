/**
 * Event API: list, read, create, update, delete, RSVP, and notification
 * preferences; persists to Firebase and delegates business rules to the C++
 * calendar service.
 */
...
/**
 * Normalize stored start/end to Unix seconds (ISO string or numeric epoch
 * seconds/ms).
 */
...
/**
 * True if [aStart, aEnd) overlaps [bStart, bEnd) in time; touching endpoints do
 * not overlap.
 */
...
/**
 * Finds same-fraternity time overlap via the in-memory C++ calendar
 * (get_all_events), not Firebase. Keeps conflict detection on the hot path for
 * API reads and avoids an extra DB round-trip.
 */
...
async function rejectIfEventNotEnded(callCppService, eventId) {
  ...
  if (nowSec < endUnix) {
    return {
      message: 'Feedback is only available after the event has ended',
      status: 403,
    };
  }
  return null;
...
/**
 * Blocks RSVP / un-RSVP / notification toggles after the event end
 * (authoritative via C++ get_event).
 */
...
/**
 * Express router for /api/events — coordinates Firebase and callCppService for
 * each operation.
 */
function createEventsRouter({
    ...
    try {
      ...
      if (userData.role !== 'Event Coordinator') {
        return res.status(403).json({
          error: 'Only event coordinators can create events',
        });
      }
      ...
      if (!fraternity) {
        return res.status(400).json({
          error: 'Your profile must include a fraternity to host events',
        });
      }
      ...
      if (conflict) {
        return res.status(409).json({
          error:
            'Your fraternity already has an event that overlaps this time: ' +
            `"${conflict.title}"`,
  ...
  router.put('/:id', authenticate, async (req, res) => {
    try {
      ...
      if (userData.role !== 'Event Coordinator') {
        return res.status(403).json({
          error: 'Only event coordinators can edit events',
        });
      }
      ...
      if (!fraternity) {
        return res.status(400).json({
          error: 'Your profile must include a fraternity to edit events',
        });
      }

      const existingResult = await callCppService('get_event', {
        id: req.params.id,
      });
      ...
      if (existing.fraternity !== fraternity) {
        return res.status(403).json({
          error: 'You can only edit events hosted by your fraternity',
        });
      }
      ...
      if (endUnix != null && nowSec >= endUnix) {
        return res.status(403).json({
          error: 'Cannot edit an event that has already ended',
        });
      }
  ...
  router.delete('/:id', authenticate, async (req, res) => {
    try {
      ...
      if (userData.role !== 'Event Coordinator') {
        return res.status(403).json({
          error: 'Only event coordinators can delete events',
        });
      }
      ...
      if (!fraternity) {
        return res.status(400).json({
          error: 'Your profile must include a fraternity',
        });
      }
      ...
      if (existing.coordinatorId !== req.user.uid) {
        return res.status(403).json({
          error: 'Only the hosting coordinator can delete this event',
        });
      }
      if (existing.fraternity !== fraternity) {
        return res.status(403).json({
          error: 'You can only delete events hosted by your fraternity',
        });
      }
      ...
      if (endUnix != null && nowSec >= endUnix) {
        return res.status(403).json({
          error: 'Cannot delete an event that has already ended',
        });
      }
  ...
  router.put('/:id/rsvp', authenticate, async (req, res) => {
    try {
      ...
      await db
          .ref(`events/${req.params.id}/attendeeIds`)
          .set(result.event.attendeeIds);
  ...
  router.put('/:id/notifications', authenticate, async (req, res) => {
    try {
      ...
      await db
          .ref(`events/${req.params.id}/notificationAttendeeIds`)
          .set(result.event.notificationAttendeeIds);
  ...
  function feedbackPath(eventId) {
    return `eventFeedback/${eventId}`;
  }
  const MAX_COMMENT_LEN = 2000;

  /**
   * GET: coordinator — C++ aggregate + rows; guest — C++ myFeedback only
   * (Firebase not used for reads).
   */
  router.get('/:id/feedback', authenticate, async (req, res) => {
    try {
      ...
      if (isOwner) {
        const cpp = await callCppService('get_event_feedback_coordinator', {
          eventId,
        });
        if (cpp.error) {
      ...
      if (profile.role !== 'Guest User') {
        return res.status(403).json({
          error: 'Only guests who attended can view personal feedback',
        });
      }
      ...
      if (!attendeeIds.includes(uid)) {
        return res.status(403).json({
          error: 'Only guests who RSVP\'d can access feedback',
        });
      }

      const cpp = await callCppService('get_event_feedback_guest', {
        eventId,
        userId: uid,
      });
      if (cpp.error) {
  ...
  /**
   * PUT: validate guest + RSVP + ended, then C++ upsert; Firebase mirrors after
   * success.
   */
  router.put('/:id/feedback', authenticate, async (req, res) => {
    try {
      ...
      if (profile.role !== 'Guest User') {
        return res.status(403).json({
          error: 'Only guest users can submit feedback',
        });
      }
      ...
      if (!attendeeIds.includes(uid)) {
        return res.status(403).json({
          error: 'You must have RSVP\'d to leave feedback',
        });
      }
      ...
      if (cpp.error || !cpp.success) {
        return res.status(400).json(
            cpp.error ?
              cpp :
              {error: 'Failed to save feedback in calendar service'},
        );
      }

      const payload = cpp.feedback;
      await db.ref(`${feedbackPath(eventId)}/${uid}`).set(payload);
