/**
 * User profile API: reads from the C++ user registry; writes update C++ first, then Firebase.
 */
...
/** Returns an Express router mounted at /api/users (get by uid, create, update, batch). */
    ...
    try {
      ...
      if (cppResult.error || !cppResult.success) {
        return res.status(400).json(
          cppResult.error ? cppResult : { error: 'Failed to persist user in calendar service' },
        );
      }
