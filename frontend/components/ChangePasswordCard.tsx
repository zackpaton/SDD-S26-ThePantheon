...
/**
 * Re-authenticates with the current password, then updates Firebase Auth
 * password.
 */
...
function firebaseErrorMessage(code: string): string {
  switch (code) {
    ...
    case 'auth/requires-recent-login':
      return (
        'For security, sign out and sign in again, then change your ' +
        'password.'
      );
  ...
  if (!hasEmailPassword) {
    return (
      <p className="text-sm text-neutral-600">
        Password changes apply to accounts that use email and password. If you
        use another sign-in method, manage your password with that provider.
      </p>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3">
      ...
      <button
        ...
        disabled={busy}
        className={
          'min-h-[44px] w-full rounded bg-purple-500 py-2.5 text-sm ' +
          'font-semibold text-white hover:bg-purple-600 ' +
          'disabled:opacity-50 sm:min-h-0 sm:w-auto sm:px-6 sm:py-2'
        }
      >
