 ...
 * When a day has more chips than fit, "+N more" opens a list (Google Calendar–style).
 */
    ...
    <div className="min-h-0 min-w-0 flex-1 overflow-auto">
      ...
      <div className="grid grid-cols-7 gap-1">
        {monthMatrix.map((week, i) =>
          week.map((day, j) => {
            const sorted = [...getEventsForDay(day)].sort((a, b) => a.startTime - b.startTime)
            ...
            return (
              <div
                key={`${i}-${j}`}
                className="flex h-24 flex-col overflow-hidden rounded border bg-white p-1 text-sm"
              >
                <div className="shrink-0 text-left text-xs font-semibold leading-none">
                  {isToday ? (
                    <span className="inline-flex h-4 w-4 items-center justify-center rounded-full bg-blue-600 text-[11px] font-semibold text-white">
                      {day.getDate()}
                    </span>
                  ) : (
                    day.getDate()
