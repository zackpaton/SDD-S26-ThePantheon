...
/**
 * Shared state for month and week calendar views: auth, events, filters, and
 * event detail modals.
 */
...
export function useCalendarBoard() {
  ...
  const [fraternitiesShown, setFraternitiesShown] = useState<string[]>([
    ...allFraternities,
  ]);
  ...
  const fetchEvents = async () => {
    try {
      ...
      if (!res.ok || !Array.isArray(data)) {
        const why = !res.ok ? res.status : 'not an array';
        console.error('Failed to fetch events:', why, data);
        setEvents([]);
        return;
      }
  ...
  /**
   * Events matching a calendar day and current filters (same logic as month
   * cells).
   */
