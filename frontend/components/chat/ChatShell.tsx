...
/**
 * Direct-messages UI: conversation list and active thread (responsive layout);
 * Firebase RTDB listeners when configured.
 */
...
/**
 * Full-height two-pane chat UI backed by `/api/chats` and optional Realtime
 * Database listeners.
 */
export default function ChatShell() {
  ...
  const [lookupCandidate, setLookupCandidate] = useState<LookupUser | null>(
    null,
  );
  ...
  /**
   * Until the conversation list includes the active thread (e.g. right after
   * opening via link).
   */
  const [peerUidUntilConvLoads, setPeerUidUntilConvLoads] = useState<
    string | null
  >(null);
  ...
  const [peerProfileError, setPeerProfileError] = useState<string | null>(
    null,
  );
  ...
  /**
   * Opens or resumes a direct chat with `peerUid` (REST); updates sidebar when
   * RTDB is off.
   */
    ...
    if (database) {
      ...
      const unsub = onValue(r, (snap) => {
        const val = snap.val() as
          | Record<string, Omit<ConversationRow, 'chatId'>>
          | null;
        if (!val) {
          setConversations([]);
          return;
        }
        const rows: ConversationRow[] = Object.entries(val).map(
          ([chatId, row]) => ({
            chatId,
            ...row,
          }),
  ...
  useEffect(() => {
    ...
    if (database) {
      ...
      const unsub = onValue(r, (snap) => {
        const val = snap.val() as
          | Record<string, Omit<ChatMessage, 'id'>>
          | null;
        if (!val) {
          setMessages([]);
          return;
  ...
  const handleLookup = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const msg =
          typeof data.error === 'string' ? data.error : 'User not found.';
        setLookupError(msg);
        return;
      }
      const composedName =
      ...
      setLookupCandidate({
        uid: data.uid,
        email: data.email,
        displayName: data.displayName || composedName || data.email,
  ...
  /**
   * Opens a thread when the URL includes `?peer=<uid>` (e.g. from an event
   * attendee profile).
   */
  useEffect(() => {
  ...
  const activeTitle =
    activeConv?.peerName || activeConv?.peerEmail || 'Conversation';
  ...
  if (!authReady) {
    return (
      <div
        className={
          'flex flex-1 items-center justify-center text-sm text-neutral-600'
        }
      >
        Loading…
      </div>
    ...
    return (
      ...
      >
        <p className="text-neutral-700">
          Please log in to send and receive chats.
        </p>
        <Link
          href="/login"
          className={
  ...
  return (
    <div
      className={
        ...
        'border-black/10 bg-white shadow-sm md:flex-row'
      }
    >
      <aside
        className={
          'flex w-full shrink-0 flex-col border-black/10 bg-[#f5f5f7] ' +
          'md:w-[min(100%,320px)] md:border-r ' +
        ...
        <div className="border-b border-black/10 px-3 py-3">
          ...
          <div className="mt-2 flex gap-1">
            <input
              ...
              className={
                'min-w-0 flex-1 rounded-md border border-black/15 bg-white ' +
                'px-2 py-2 text-base outline-none focus:ring-2 ' +
                'focus:ring-blue-500 sm:py-1.5 sm:text-sm'
              }
            />
            <button
              type="button"
              ...
              className={
                'shrink-0 rounded-md bg-blue-600 px-3 py-1.5 text-sm ' +
          ...
          {lookupCandidate && (
            <div
              className={
                'mt-2 rounded-lg border border-black/10 bg-white p-2 ' +
                'shadow-sm'
              }
            >
              ...
              <button
                ...
                className={
                  'mt-2 w-full rounded-md bg-blue-600 py-1.5 text-sm ' +
                  'font-medium text-white hover:bg-blue-700 disabled:opacity-50'
                }
              >
                    ...
                    >
                      <span
                        className={
                          ...
                          'rounded-full bg-blue-600 text-xs font-semibold ' +
                          'text-white'
                        }
                      >
                        {initials(title)}
      ...
      >
        {!activeChatId ? (
          ...
          >
            <p className="text-sm">
              Select a conversation or find someone by email
            </p>
          </div>
        ) : (
          <>
            <header
              className={
                'flex shrink-0 flex-col border-b border-black/10 px-3 ' +
                'pb-3 pt-3 sm:px-4'
              }
            >
              <div className="flex items-center">
                <button
                  ...
                  className={
                    'mr-2 flex min-h-10 min-w-10 shrink-0 items-center ' +
                    'justify-center rounded-lg text-lg text-neutral-700 ' +
                    'hover:bg-black/5 md:hidden'
                  }
              ...
              <button
                ...
                className={
                  'mt-2 w-full rounded-lg border border-black/15 ' +
                  'bg-purple-400 px-3 py-2 text-sm font-medium ' +
                  'text-neutral-900 hover:bg-purple-500 ' +
            ...
            <div
              className={
                'min-h-0 flex-1 overflow-y-auto bg-[#e8e8ed] px-3 py-3'
              }
            >
                  ...
                  messages.map((m) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={
                            'max-w-[85%] rounded-[18px] px-3 py-2 text-sm ' +
                            'shadow-sm ' +
                            (mine ?
            ...
            <footer
              className={
                'shrink-0 border-t border-black/10 bg-white/90 px-3 ' +
            ...
            >
              <div className="mx-auto flex max-w-3xl items-end gap-2">
                <textarea
                  ...
                  rows={1}
                  className={
                    'max-h-32 min-h-[44px] flex-1 resize-y rounded-xl ' +
                    'border border-black/15 bg-white px-3 py-2.5 ' +
                    'text-base outline-none focus:ring-2 focus:ring-blue-500 ' +
                    'sm:min-h-[40px] sm:py-2 sm:text-sm'
                  }
                />
                <button
                  type="button"
                  ...
                  className={
                    'min-h-[44px] shrink-0 rounded-full bg-[#007aff] px-4 ' +
                    'py-2 text-sm font-medium text-white hover:bg-blue-600 ' +
                    'disabled:opacity-40 sm:min-h-0'
                  }
                >
                  Send
                </button>
          ...
          >
            <div className="mb-3 flex items-start justify-between gap-2">
              ...
              >
                {peerProfileLoading ?
                  'Profile' :
                  `${peerProfile?.firstName || ''} ${
                    peerProfile?.lastName || ''
                  }`.trim() ||
                    activeTitle ||
                    'Profile'}
              </h3>
