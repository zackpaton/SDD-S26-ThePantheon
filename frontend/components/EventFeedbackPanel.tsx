...
/**
 * Post-event feedback: guests submit up/down + comment; coordinators see
 * totals and RSVP guest comments.
 */
...
export default function EventFeedbackPanel(props: Props) {
  ...
  const [coordinatorData, setCoordinatorData] =
    useState<CoordinatorFeedbackPayload | null>(null);
  const [myFeedback, setMyFeedback] = useState<
    GuestFeedbackPayload['myFeedback']
  >(null);
  ...
  const loadCoordinator = useCallback(async () => {
    ...
    const res = await fetch(
      `${API_ORIGIN}/api/events/${props.eventId}/feedback`,
      {
            ...
            {coordinatorData.entries.length === 0 ? (
              ...
              <ul
                className={
                  ...
                  'border-neutral-200 bg-neutral-50 p-2 text-sm'
                }
              >
  ...
  return (
    <div className="mt-4 border-t border-neutral-200 pt-3">
      ...
      <p className="mb-2 text-xs text-neutral-600">
        Share how it went (one vote per person; you can update your feedback
        anytime).
      </p>
        ...
        <>
          <div className="mb-2 flex gap-2">
            <button
              ...
              className={
                'flex-1 rounded border-2 px-3 py-2 text-sm font-medium ' +
                'transition-colors ' +
                (draftVote === 'up' ?
          ...
          <textarea
            ...
            maxLength={2000}
            className={
              'mb-2 w-full rounded border border-neutral-300 px-2 py-1.5 ' +
              'text-sm'
            }
          ...
          <button
            ...
            className={
              'w-full rounded bg-purple-600 px-4 py-2 text-sm font-medium ' +
              'text-white hover:bg-purple-700 disabled:cursor-not-allowed ' +
              'disabled:opacity-50'
            }
          >
            {saving ?
