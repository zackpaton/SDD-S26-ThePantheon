...
/**
 * Registration form: creates a Firebase user then POSTs profile (role,
 * fraternity) to the backend.
 */
...
/**
 * Validates passwords, creates the auth account, syncs `/api/users`, and sends
...
export default function SignUpForm() {
  ...
  /**
   * Runs client-side checks, Firebase signup, and backend user creation in
   * sequence.
   */
  const handleSignup = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const fallback =
          'Could not save your profile. Try signing in, or contact support ' +
          'if this continues.';
        setFormError(getApiErrorMessage(body, fallback));
        return;
      }
        ...
        <div>
          <button
            ...
            className={
              ...
              'text-white hover:bg-purple-700 disabled:opacity-60 ' +
              'sm:min-h-0 sm:py-2'
            }
          >
