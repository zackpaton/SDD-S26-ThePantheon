/**
 * Positions overlapping timed events in a single day column (week view): greedy column packing
 * plus per-event width based on concurrent overlap count (Google Calendar–style columns).
 */
...
export function getVisibleDayInterval(
  event: CalendarEvent,
  day: Date,
): { startMin: number; endMin: number; topPct: number; heightPct: number } | null {
 ...
 */
export function layoutWeekDayEvents(events: CalendarEvent[], day: Date): Map<
  string,
...
> {
  ...
  const out = new Map<string, { topPct: number; heightPct: number; leftPct: number; widthPct: number }>()
